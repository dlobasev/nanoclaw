 ...
 */
async function resolveOwningAgentFromEvent(
  event: InboundEvent,
  ...
  if (parsed.type === 'reaction' && parsed.messageId) {
    const owner = await findOwningAgent(event.channelType, event.platformId, String(parsed.messageId));
    if (owner) return owner.agentGroupId;
  }
  // Reply event: replyTo.messageId nested in content.
  if (parsed.replyTo?.messageId) {
    const owner = await findOwningAgent(event.channelType, event.platformId, String(parsed.replyTo.messageId));
    if (owner) return owner.agentGroupId;
  }
  return null;
}
...
export async function routeInbound(event: InboundEvent): Promise<void> {
  ...
  const owningAgentId = await resolveOwningAgentFromEvent(event, parsed);
  ...
  for (const agent of agents) {
    ...
    // Owning-agent short-circuit: a reaction/reply addressed to this agent's
    // prior message engages it directly, bypassing pattern evaluation. Falls
    // through to normal engage for every other case.
    const ownerEngage = ownerIsWired && agent.agent_group_id === owningAgentId;
    const engages = ownerEngage || (await evaluateEngage(agent, messageText, isMention, mg, effectiveThreadId));
