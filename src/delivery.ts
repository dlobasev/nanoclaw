...
async function drainSession(session: Session): Promise<void> {
  ...
  for (const msg of pending) {
    try {
      ...
      if (platformMsgId && msg.channelType && msg.platformId && msg.kind !== 'system' && msg.channelType !== 'agent') {
        await recordOutboundMessage(msg.channelType, msg.platformId, platformMsgId, session.agent_group_id, session.id);
      }
